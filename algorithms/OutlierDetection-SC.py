# -*- coding: utf-8 -*-
"""OutlierDetection-SC.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15rJPOrHRQ5kvxGX5QoPDxfemxDTjmJhA
...
# data = pd.read_csv('./sample_data/friendship_clean.csv', header=None)
# print(data)

# plt.scatter(data[1], data[2])  # Access first and second column for x and y coordinates
# plt.title('Original plotting of Synthetic data')
# plt.show()
...
# # data = pd.read_csv('./sample_data/friendship_clean.csv', header=None)
# # gender_data = pd.read_csv('./sample_data/group_clean.csv', header=None)

# # # Merge the gender information with the friendship data based on node numbers
# # data_with_gender = pd.merge(data, gender_data, left_on=0, right_on=0)

# # # Generate synthetic data
# # # Assuming nodes range from 1 to maximum node number in the dataset
# # nodes = data_with_gender.iloc[:, :2].values
# # weights = data_with_gender.iloc[:, 2].values
# # gender = data_with_gender.iloc[:, 3].values  # Assuming gender is in the fourth column

# # # Plot the data
# # plt.figure(figsize=(8, 6))

# # # Plot male nodes
# # male_nodes = nodes[gender == 0]
# # male_weights = weights[gender == 0]
# # plt.scatter(male_nodes[:, 0], male_nodes[:, 1], s=male_weights, c='blue', alpha=0.5, label='Male')

# # # Plot female nodes
# # female_nodes = nodes[gender == 1]
# # female_weights = weights[gender == 1]
# # plt.scatter(female_nodes[:, 0], female_nodes[:, 1], s=female_weights, c='red', alpha=0.5, label='Female')

# # plt.xlabel('Node 1')
# # plt.ylabel('Node 2')
# # plt.title('Synthetic Data for Friendship Network with Gender Information')
# # plt.legend()
# # plt.grid(True)
# # plt.show()
...
#     i = group.index[group[0] == data.iloc[m, 0]][0]
#     j = group.index[group[0] == data.iloc[m, 1]][0]
#     if data.iloc[m, 2] == 1:
#         G.add_edge(i, j)
...
data_array = X
mini_datasets_features, mini_datasets = generate_mini_datasets(X, 10, X.shape[0])
...
#       #     # cluster_labels.append(np.zeros(len(mini_dataset)))
#       #     second_smallest_eigenvalues.append(0)
#       #     # eigenvalue_counts[0] += 1

#       # except Exception as e:
#       #   print(f"Error occurred for mini dataset {i}: {e}")
#       #   second_smallest_eigenvalues.append(0)
...
final_outliers = rank_outliers(outlier_tuples_desc, 6)
